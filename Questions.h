...
void askVehicle(bool editing, string &type, int &yearOfManufacture, string &make, string &model, string &trimLevel, string &plateNum, string &color)
{
     ...
     {
          cout << endl
               << "Enter model of the vehicle [e.g. Supraaa]: ";
          cin >> model;
     ...
     do
     {
          ...
          cout << endl
               << "Enter plate number of the vehicle [e.g. ABC123]: ";
          cin >> plateNum;
     ...
     {
          cout << endl
               << "Enter color of the vehicle [e.g. Blsck]: ";
          cin >> color;
...
void askCardNum(string &cardNum, bool editing)
{
     bool is_err;
     do
     {
          ...
          if (editing && cardNum == "-1")
               break;

          is_err = !isValidCardNum(cardNum);

          if (is_err)
               cerr << endl
                    << "Error: Invalid Card Num" << endl;
     } while (is_err);
}

void askCVVcode(string &cvvCode, bool editing)
{
     bool is_err;
     do
     {
          ...
          if (editing && cvvCode == "-1")
               break;

          is_err = !isValidCVVcode(cvvCode);

          if (is_err)
               cerr << endl
                    << "Error: Invalid CVV Code" << endl;
     } while (is_err);
}

void askCardDate(string &carddate, bool editing)
{
     bool is_err;
     do
     {
          cout << endl
               << "Enter Card Date (MMYY) : ";
          cin >> carddate;

          if (editing && carddate == "-1")
               break;

          is_err = !isValidCardDate(carddate);

          if (is_err)
               cerr << endl
                    << "Error: Invalid Card Date" << endl;
     } while (is_err);
}

void askPaypalEmail(string &email, bool editing)
{
     ...
     do
     {
          ...
          is_err = !isValidPaypalEmail(email);

          if (is_err)
               cerr << endl
                    << "Error: Invalid Paypal Email" << endl;
     } while (is_err);
}
