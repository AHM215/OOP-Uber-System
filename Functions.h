...
bool isPhoneNumValid(string phoneNum)
{
    ...
    regex phoneNumRegex("^01[0125][0-9]{8}");

    return regex_match(phoneNum, phoneNumRegex);
}
...
bool isValidPlateNum(string plateNum)
{
    // AA-111
    // AAA-1111
    regex plateNumRegex("[A-Z]{3}[0-9]{3}");
    return regex_match(plateNum, plateNumRegex);
}

bool isValidCardNum(string cardNum)
{
    bool valid = cardNum.length() == 16;
...
bool isValidCVVcode(string cvvCode)
{
    bool valid = cvvCode.length() == 3;
...
bool isValidCardDate(string cardDate)
{
    ...
    // Extract the month and year from the cardDate
    int month = stoi(cardDate.substr(0, 2));       // first two digits
    int year = stoi(cardDate.substr(2, 2)) + 2000; // last two digits

    // Get the current month and year
    time_t now = time(0);               // current time
    tm *ltm = localtime(&now);          // local time structure
    int currMonth = 1 + ltm->tm_mon;    // current month (1-12)
    int currYear = 1900 + ltm->tm_year; // current year
